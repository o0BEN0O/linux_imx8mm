...
static u64 field_multiplier(u64 field)
{
	if ((field | (field - 1)) & ((field | (field - 1)) + 1))
		printk("bad bitfield mask\n");
	return field & -field;
}
static u64 field_mask(u64 field)
{
			  ...
			  const u8 mode_req, bool nowait)
{
	...
	err = regmap_read_poll_timeout(priv->map, MCP25XXFD_CAN_CON, con,
				       ...
				       10000, 1000000);
	if (err) {
		mode = FIELD_GET(MCP25XXFD_CAN_CON_OPMOD_MASK, con);
	...
	netdev_info(priv->ndev,
		"Controller enter mode %s Mode (%u) and stays in %s Mode (%u).\n",
		mcp25xxfd_get_mode_str(mode_req), mode_req,
		mcp25xxfd_get_mode_str(mode), mode);
...
static int mcp25xxfd_chip_pinctrl_init(const struct mcp25xxfd_priv *priv)
{
	...
	//val = MCP25XXFD_IOCON_PM0 | MCP25XXFD_IOCON_TRIS0 | MCP25XXFD_IOCON_TRIS1 | MCP25XXFD_IOCON_PM1;
	//regmap_write(priv->map, MCP25XXFD_IOCON, val);
	regmap_read(priv->map, MCP25XXFD_IOCON, &val);
	netdev_info(priv->ndev,"IOCON = 0x%8x\n",val);
...
#endif
	//mode = MCP25XXFD_CAN_CON_MODE_INT_LOOPBACK;
...
static int mcp25xxfd_chip_start(struct mcp25xxfd_priv *priv)
{
 ...
 out_chip_set_mode_sleep:
	...
	netdev_err(priv->ndev,
				    "mcp25xxfd_chip_start error and set sleep mode\n");
...
static irqreturn_t mcp25xxfd_irq(int irq, void *dev_id)
{
	...
	do {
		...
		netdev_info(priv->ndev, "intf_pending = 0x%x\n",intf_pending);
...
static int mcp25xxfd_probe(struct spi_device *spi)
{
	...
	priv->can.ctrlmode_supported = CAN_CTRLMODE_ONE_SHOT | CAN_CTRLMODE_FD | CAN_CTRLMODE_LOOPBACK |
