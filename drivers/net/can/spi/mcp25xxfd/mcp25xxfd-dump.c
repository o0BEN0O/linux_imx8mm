...
static void mcp25xxfd_dump_ram_rx_obj_one(const struct mcp25xxfd_priv *priv, const struct mcp25xxfd_dump_regs *regs, const struct mcp25xxfd_rx_ring *ring, const struct mcp25xxfd_hw_rx_obj_canfd *hw_rx_obj, u8 n)
{
	pr_info("RX Object: 0x%02x (0x%03x)%s%s%s%s%s%s\n",
		...
		mcp25xxfd_get_rx_head(ring) == n ? "  priv-HEAD" : "",
...
static void mcp25xxfd_dump_ram_rx_obj(const struct mcp25xxfd_priv *priv, const struct mcp25xxfd_dump_regs *regs, const struct mcp25xxfd_dump_ram *ram, const struct mcp25xxfd_rx_ring *ring)
{
	int i;

	pr_info("\nRX Overview:\n");
	pr_info("%16s = 0x%02x    0x%02x    0x%08x\n", "head (c/p)", mcp25xxfd_dump_get_rx_head(priv, regs), mcp25xxfd_get_rx_head(ring), ring->head);
	pr_info("%16s = 0x%02x    0x%02x    0x%08x\n", "tail (c/p)", mcp25xxfd_dump_get_rx_tail(priv, regs), mcp25xxfd_get_rx_tail(ring), ring->tail);
	pr_info("\n");
	...
	for (i = 0; i < mcp25xxfd_dump_get_rx_obj_num(priv, regs); i++) {
		...
		mcp25xxfd_dump_ram_rx_obj_one(priv, regs, ring, hw_rx_obj, i);
	}
}
